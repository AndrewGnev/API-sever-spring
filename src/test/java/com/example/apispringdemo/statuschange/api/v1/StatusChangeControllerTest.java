...
public class StatusChangeControllerTest {
    ...
    public void whenGetIsCalled_thenReturnsRightStatistics() {
        List<StatusChangeEntity> entityList = Arrays.asList(
                ...
                new StatusChangeEntity(2L, new UserEntity(2L,"v", "f"), OnlineStatus.ONLINE, 1L),
                new StatusChangeEntity(3L, new UserEntity(3L,"w", "h"), OnlineStatus.ONLINE, 1L)
        );

        List<StatusChangeDto> dtoList = Arrays.asList(
                new StatusChangeDto(1L, new UserEntity(1L,"u", "e"), OnlineStatus.ONLINE, 1L),
                new StatusChangeDto(2L, new UserEntity(2L,"v", "f"), OnlineStatus.ONLINE, 1L),
                new StatusChangeDto(3L, new UserEntity(3L,"w", "h"), OnlineStatus.ONLINE, 1L)
        );

        when(service.getStatistics(any(OnlineStatus.class), anyLong())).thenReturn(entityList);

        assertEquals(dtoList, controller.get(OnlineStatus.ONLINE, 0L));
